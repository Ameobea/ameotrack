...
TYPES.basic_rocket = function(vector, position, child_fireworks) {
  return {
    ...
    vector: [
      Math.random() * ((Math.random() > 0.5) * -1) * 4,
      Math.random() * 4 + 5,
    ],
    trail_width: 3,
    trail_color: 'red',
    ...
    trail: {
      ...
      trail_generator: function(position, tick, width, vector, parent_vector) {
        ...
        if (tick % 2 == 0) {
          return TYPES.random_test_trail(
            [(Math.random() * 2 - 1) / 4.5, Math.random() * -1],
            position
          );
        } else {
          return false;
...
function spawn_firework(properties) {
  if (properties.x) {
    firework = new Path.Circle(
      new Point(properties.x, view.bounds.height - properties.trail_width - 5),
      properties.trail_width
    );
  } else {
...
function step_firework(firework, j) {
  firework.vector[1] = firework.vector[1] - GRAVITY * firework.mass * 0.0055;
  ...
  if (typeof firework.color_generator == 'function') {
    color_mod = firework.color_generator(
      firework.fillColor,
      firework.strokeColor
    );
  ...
  if (typeof firework.trail == 'object' && quality > 1) {
    if (
      firework.trail.trail_generator(
        pos_cache,
        firework.tick,
        firework.trail_width,
        firework.trail.vector_generator(firework.trail.number, k),
        firework.vector
      )
    ) {
  ...
  if (firework.tick >= firework.explosion_delay) {
    if (firework.child_fireworks) {
      for (var l = 0; l < firework.child_fireworks.number; l++) {
        if (
          firework.child_fireworks.child_generator(
            l,
            firework.child_fireworks.vector_generator(
              firework.child_fireworks.number,
              l
            ),
            pos_cache
          )
