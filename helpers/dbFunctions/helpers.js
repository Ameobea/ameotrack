...
helpers.renameJournal = function(oldPath, newPath, uploadDate, callback){
	fs.exists("./journals/".concat(uploadDate.getFullYear(), "/"), function(yearFolderExists){
		...
		fs.exists("./journals/".concat(uploadDate.getFullYear(), "/", uploadDate.getMonth()+1, "/"), function(monthFolderExists){
			if(!monthFolderExists){
				fs.mkdirSync("./journals/".concat(uploadDate.getFullYear(), "/", uploadDate.getMonth()+1, "/"));
			}
			fs.exists("./journals/".concat(uploadDate.getFullYear(), "/", uploadDate.getMonth()+1, "/", uploadDate.getDate(), "/"), function(dayFolderExists){
				if(!dayFolderExists){
					fs.mkdirSync("./journals/".concat(uploadDate.getFullYear(), "/", uploadDate.getMonth()+1, "/", uploadDate.getDate(), "/"));
				}
